...
export function GlobalNavigation ({ isAuthenticated, onLogout }: GlobalNavigationProps) {

  const pathname = usePathname();

  return (
    <nav aria-labelledby="global-navigation" className="flex bg-background-default fixed bottom-0 left-0 right-0 md:top-0 md:bottom-auto z-50" role="banner">
          ...
          <ul className="flex w-full items-center justify-around md:w-auto md:justify-end md:gap-6">
            ...
            { isAuthenticated ? (
              <>
                ...
                <li>
                  <IconButton icon="logout" label="ログアウト" onClick={onLogout}/>
                </li>
              </>
            ):(
              <>
